...
if (Grocery.Option == 'fruits' || Grocery.Option == 'Fruits') {
    ...
    let ResFruit = await inquirer.prompt([{
            name: 'Fruit_list',
            type: 'checkbox',
    ...
    let length = ResVeg.Veg_list.length;
    let list = ResVeg.Veg_list;
    let quantity2 = ResVeg.Quantity;
...
if (Total_Bill > 150) { // Discount 40% if bill is above 150
    ...
    let DiscountedBill = Total_Bill * 40 / 100;
...
// }
let Payarr = ['Paypal', 'Bitcoin', 'Erc20', 'Pioneer'];
for (let i = 0; i < Payarr.length; i++) {
    console.log(`${i + 1}- ${Payarr[i]}`);
}
let Payres = await inquirer.prompt([{
        name: 'Pmeth',
        type: 'list',
        message: 'Select One Of the payment Method',
        choices: ['Paypal', 'Erc20', 'Bitcoin', 'Pioneer']
